...
{
    public class CrearUsuarioHandler : IRequestHandler<CrearUsuarioCom, int>
    {
        //private readonly ITipoIdentificacion tipoIdentificacion;

        private readonly ICacheServicio cacheServicio;

        public CrearUsuarioHandler(ICacheServicio cacheServicio)
        {
            this.cacheServicio = cacheServicio;
        }
        public async Task<int> Handle(CrearUsuarioCom request, CancellationToken cancellationToken)
        {
